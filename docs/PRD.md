...
osmcli は、OpenStreetMap 由来のジオコーディング・OSM要素検索・ルーティングを、非対話・安定I/OのCLIとして束ね、コーディングAIエージェントが繰り返し呼び出して合成できるようにするツールです。
...
コーディングAIエージェント（ReActエージェント等）は、以下の理由から地理空間タスクを苦手としています：

1. **ハルシネーション:** LLMは正確な座標や最新のPOI（施設情報）を確実に記憶・出力できない。
2. **複雑性:** GISデータを扱うには、複数のAPI（Nominatim, Overpass, Routing）や複雑なライブラリ（GDAL, GeoPandas）を使いこなす必要があり、貴重なコンテキストトークンを消費し、エラー率を高める。
3. **統一インターフェースの欠如:** データの「取得（APIクライアント）」と「加工（GIS演算）」を、パイプラインで接続しやすい形で統合した標準的なCLIが存在しない。
...
osmcli は、この摩擦を取り除き、AIエージェントが「地名→領域(AOI)→POI集計/抽出→移動/到達圏」といった一連の地理タスクを、短いコマンド列で確実に遂行できる状態を作ります。

## 3. ターゲット層

- **プライマリ:** シェル実行能力を持つコーディングAIエージェント。
- **セカンダリ:** 位置情報アプリケーションを開発するエンジニア、手軽に地理データを確認したいデータアナリスト。

## 4. コア原則 (Core Principles)

1. **Agent-First:** 決定論的な出力、標準化された終了ステータス、機械可読なフォーマット（JSON/GeoJSON）を徹底する。
2. **UNIX Philosophy:** 「一つのことをうまくやる」。あるプログラムの出力が、次のプログラムの入力になることを前提とする。テキストストリーム（GeoJSON Lines）を活用する。
3. **Semantic Wrapper:** 裏側にあるAPI（Nominatim, Overpass, Valhalla）の複雑さを、直感的で意味のあるコマンド体系の下に隠蔽する。
...
- osmcli reverse --lat 35.6768601 --lon 139.7638947 --format json
...
- osmcli poi count --tag amenity=cafe --within "東京都台東区" --format json
- osmcli poi count --preset cafe --within @aoi.geojson --format json
...
- osmcli poi fetch --preset cafe --within "東京都台東区" --format geojson
- osmcli poi fetch --tag amenity=cafe --within @aoi.geojson --format ndjson | jq ...
...
- osmcli route --from "東京駅" --to "浅草寺" --mode pedestrian --format json
- osmcli isochrone --from "東京駅" --minutes 10,20 --mode pedestrian --format geojson
...
- osmcli isochrone --from "東京駅" --minutes 10,20 --mode pedestrian --format geojson \
  | osmcli poi count --preset cafe --within - --format json
...
- 地名→AOI→カフェ数が、3コマンド以内で再現できる
  - 例: osmcli poi count --preset cafe --within "東京都台東区" --format json
- AOI GeoJSON を一度出力し、それを入力として再利用できる
  - 例: osmcli aoi resolve "東京都台東区" --format geojson > aoi.geojson
    osmcli poi count --preset cafe --within @aoi.geojson --format json
